...
class SingleRenderer extends BaseRenderer {
    render(entry) {
        ...
        ele.append(logo);
				ele.removeChild(ele.querySelector('.tweet-medias'));
        return ele;
    }
...
class SingleLayout extends BaseLayout {
    ...
    next() {
        let currElement = this.container.children[this.index];
        currElement.style.display = 'none';
        this.index = (this.index + 1) % this.container.children.length;
        let nextElement = this.container.children[this.index];
