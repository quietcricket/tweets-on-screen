...
class BaseRenderer {
    constructor(custom_emojis = {}) {
        this.custom_emojis = { "oneteam": "OneTeam_V2Design_Evergreen.png" };
        for (let k in custom_emojis) {
            this.custom_emojis[(k + "").toLowerCase()] = custom_emojis[k];
        }
    }
    render(entry) {
    ...
    hilightHashtags(entities, text) {
        ...
        for (let h of entities.hashtags) {
            ...
            text = text.replace(`#${h.text}`, `<span class="hilight">#${h.text}</span>${emoji}`);
        }
        return text;
    }
